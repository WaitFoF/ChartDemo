...
void CChartSurfaceSerie::DrawAll(CDC* pDC)
{
	...
	if (m_FillStyle == fsSolid) {
		//NewBrush.FromHandle((HBRUSH)GetStockObject(NULL_BRUSH));
		NewBrush.CreateSolidBrush(m_SerieColor);
		//Graphics gr(pDC->GetSafeHdc());
		//SolidBrush rh(Color(50,0,0,0)); // Alpha，Red，Blue，Green 
		//SolidBrush NewBrush = new SolidBrush(Color(128, GetRValue(77), GetGValue(77), GetBValue(0))); // 透明度 128
	}
	else
	{
		//带花纹的画刷
		...
		*/
		...
		switch (m_FillStyle)
		{
		case fsHatchDownDiag:  //45度向下，从左到右的阴影。
			nIndex = HS_FDIAGONAL;
			break;
		case fsHatchUpDiag:    //45度向上，从左到右的阴影。
			nIndex = HS_BDIAGONAL;
			break;
		case fsHatchCross:     //纵横交叉的阴影。
			nIndex = HS_CROSS;
			break;
		case fsHatchDiagCross://45度交叉的阴影。
			nIndex = HS_DIAGCROSS;
			break;
		case fsHatchHorizontal://水平阴影。
			nIndex = HS_HORIZONTAL;
			break;
		case fsHatchVertical://垂直阴影。
			nIndex = HS_VERTICAL;
			break;
		}
