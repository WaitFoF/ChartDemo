...
//	m_fMinXValue = (float)_tstof(szBuff);
	...
	//获取:Associated vert axis,Associated horiz axis
	...
	//设置线型：Line、 Sine wave、Random data
	if ( ((CButton*)GetDlgItem(IDC_LINEDATA_RADIO))->GetCheck() == 1)
	{
		m_iDataType = 0;//Line
	    //获取 Line Slope ,Line Offset
...
BOOL CSeriesPropDlg::OnInitDialog() 
{
	...
	GetDlgItem(IDC_MAXXVALUE_EDIT)->SetWindowText(_T("10.0")); 

	GetDlgItem(IDC_DATAPARAM1_EDIT)->SetWindowText(_T("1.0"));
	GetDlgItem(IDC_DATAPARAM2_EDIT)->SetWindowText(_T("0.0"));
	GetDlgItem(IDC_POINTSNUMBER_EDIT)->SetWindowText(_T("50")); 
...
void CSeriesPropDlg::OnSelectRandomData() 
{
	...
	GetDlgItem(IDC_DATAPARAM2_EDIT)->SetWindowText(_T("10"));
}
...
void CSeriesPropDlg::OnSelectSineData() 
{
	...
	GetDlgItem(IDC_DATAPARAM1_EDIT)->SetWindowText(_T("10.0"));
