...
void CChartDemoDlg::OnLeftAxisRadio()
{
	...
	//坐标轴是否自动——是，勾选Automatic多选框；否，不勾选Automatic多选框
...
void CChartDemoDlg::OnAddseries() 
{
	...
	//判断选择的三种绘图类型：Line ，Points ， Surface 
	...
	case 0:
		{
		    //bSecondHorizAxis：x坐标轴是选择top还是bottom
			bool bSecondHorizAxis = (PropertiesDlg.m_iHorizAxis == 1);
			//bSecondVertAxis：y坐标轴是选择left还是right
			bool bSecondVertAxis = (PropertiesDlg.m_iVertAxis == 1);
			//创建线对象 CChartLineSerie继承自CChartXYSerie
	...
	case 2:
		{
			//bSecondHorizAxis：x坐标轴是选择top(1)还是bottom(0)
			bool bSecondHorizAxis = (PropertiesDlg.m_iHorizAxis == 1);//bottom：false；top：true 
			//bSecondVertAxis：y坐标轴是选择left(0)还是right(1) 
			bool bSecondVertAxis = (PropertiesDlg.m_iVertAxis == 1);
			//创建线对象 CChartSurfaceSerie继承自CChartXYSerie                
			CChartSurfaceSerie* pSurfSeries = m_ChartCtrl.CreateSurfaceSerie(bSecondHorizAxis, bSecondVertAxis);//（bottom，right)=(false,true)
			pSeries = pSurfSeries;

			CSurfacePropDialog SurfPropDlg;
			SurfPropDlg.DoModal();
			//获取使用的线型
	         //fsHatchDownDiag:    //45度向下，从左到右的阴影。
		     //fsHatchUpDiag:      //45度向上，从左到右的阴影。
		     //fsHatchCross:       //纵横交叉的阴影。
		     //fsHatchDiagCross:   //45度交叉的阴影。
		     //fsHatchHorizontal:  //水平阴影。
		     //fsHatchVertical:    //垂直阴影。
